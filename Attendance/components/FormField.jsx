...
const FormField = ({ title, editable, value, placeholder, handleChangeText, otherStyles,smallStyle, ...props }) => {
  ...
  return (
    <View className={`space-y-2 m-2 ${otherStyles}`}>
      ...
      <View className="border-2 border-secondary w-full h-16 px-4 rounded-2xl focus:border-gray-300 items-center flex-row">
        <TextInput className={`flex-1 font-psemibold text-base text-black-200 ${smallStyle}`}
        value={value}
